...
for case_index in eachindex(L_array)
    ...
    wave_model_normalizer =  wave_model_normalizer_array[case_index]
    ...
    for n in eachindex(omega_m_cases)
        ...
        params = @SVector [η, ε, Omegape, omega_m, a, dPhi, wave_model_coeffs, wave_model_normalizer];
        prob = ODEProblem(eom!, ~, tspan, params);
        prob_func = ((prob,i,repeat) -> remake(prob, u0 = h0[i,:], p = @SVector [η, ε, Omegape, omega_m, a, dPhi, wave_model_coeffs, wave_model_normalizer]))
