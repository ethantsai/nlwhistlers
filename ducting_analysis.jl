...
plot!(E_bins, fit(Histogram, initial_E[loss_range], logrange(ELo, EHi, Esteps+1)).weights)
...
fit(Histogram, initial_E[loss_range], logrange(ELo, EHi, Esteps+1))
...
for traj in sol
    ...
    @views push!(allBw, wave_normalizer*wave_model_array[1].(rad2deg.(vars[:,5])))
end
